...
const cache = { content: null, messages: null, tilbod: null, shas: {}, uploads: new Map() };

// Tilbodslageret: løpenummer, prisbank som veks for kvar lagra varelinje,
// og sjølve tilboda. Strukturen er lagd for at ein språkmodell seinare skal
// kunne lære av tidlegare tilbod.
function tomtTilbodslager() {
  return { teljar: 1000, prisbank: [], tilbod: [] };
}
...
async function init() {
  ...
  if (remote) {
    ...
    const t = await ghGetFile('tilbod.json');
    if (t) {
      cache.tilbod = JSON.parse(t.buffer.toString('utf8'));
      cache.shas['tilbod.json'] = t.sha;
    } else {
      cache.tilbod = tomtTilbodslager();
    }
  } else {
    ...
    const t = localRead('tilbod.json');
    cache.tilbod = t ? JSON.parse(t.toString('utf8')) : tomtTilbodslager();
  }
}

function getContent() { return cache.content; }
function getMessages() { return cache.messages; }
function getTilbod() { return cache.tilbod; }
...
async function saveTilbod(data) {
  return iKoe(async () => {
    sjekkSkrivbar();
    const buf = Buffer.from(JSON.stringify(data, null, 2));
    if (remote) await ghPutFile('tilbod.json', buf, 'Oppdater tilbod');
    else localWrite('tilbod.json', buf);
    cache.tilbod = data;
  });
}

// Les-endre-skriv på tilbodslageret, heilt inne i skrivekøa.
//
// Tilbod, oppsett og prisbank ligg i same fila. Les ein rute lageret utanfor
// køa og skriv det tilbake etterpå, kan to samstundes lagringar - to tilbod,
// eller eit tilbod og ei oppsettsendring - overskrive kvarandre. Her ser `fn`
// alltid det ferskaste lageret, og ingen andre skriv før ho er ferdig.
async function endraTilbod(fn) {
  return iKoe(async () => {
    sjekkSkrivbar();
    const lager = JSON.parse(JSON.stringify(cache.tilbod || tomtTilbodslager()));
    const svar = await fn(lager);
    const buf = Buffer.from(JSON.stringify(lager, null, 2));
    if (remote) await ghPutFile('tilbod.json', buf, 'Oppdater tilbod');
    else localWrite('tilbod.json', buf);
    cache.tilbod = lager;
    return svar;
...
module.exports = {
  ...
  getTilbod, saveTilbod, endraTilbod,
